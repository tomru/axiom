...
import { markdownRouteToPath, getVersion } from 'style-guide/utils/markdown-document';
...
export function buildNavigationItems(activePath, openPath) {
  return getVersion(activePath).children.map((item) => {
    return buildNavigationItem(item, activePath, openPath, [getVersion(activePath).id])
  });
}
