...
describe('DotPlot (utils)', () => {
  ...
  describe('with no mouse over', () => {
    it('does not filter colors', () => {
      expect(getDotColors(null, [], 0, ['red', 'terra-form'])).toEqual([
        'red',
        'terra-form',
      ]);
    });
    ...
    it('hides values', () => {
      expect(isValueHidden(null, [], 0, ['red', 'terra-form'])).toEqual(true);
    });

    it('does not fade the lines', () => {
      expect(isLineFaded(null, [], 0, ['red', 'terra-form'])).toEqual(false);
    });
  });

  describe('with mouse over', () => {
    it('fades the lines', () => {
      expect(isLineFaded(0, [], 0, ['red', 'terra-form'])).toEqual(true);
    });

    describe('on other rows', () => {
      it('does not bold the values', () => {
        expect(isValueStrong(1, ['red'], 2, ['red'])).toEqual(false);
      });
    });

    describe('on the target row', () => {
      it('does not bold the values', () => {
        expect(isValueStrong(1, ['red'], 1, ['red'])).toEqual(true);
      });
    });
    ...
    describe('on a single data point', () => {
      describe('on other rows', () => {
        it('hides dots of different colors', () => {
          expect(isDotHidden(1, ['red'], 2, ['terra-form'])).toEqual(true);
        });

        it('shows dots of the same color', () => {
          expect(isDotHidden(1, ['red'], 2, ['red'])).toEqual(false);
        });

        it('filters stacked dots to the hovered color', () => {
          expect(
            getDotColors(1, ['red'], 2, ['red', 'terra-form', 'giant-leap'])
          ).toEqual(['red']);
        });

        it('does not filter single dots of different colors', () => {
          expect(getDotColors(1, ['giant-leap'], 2, ['red'])).toEqual(['red']);
        });

        it('shows values of the same color for single dots', () => {
          expect(isValueHidden(1, ['red'], 2, ['red'])).toEqual(false);
        });

        it('shows values of the same color for stacked dots', () => {
          expect(isValueHidden(1, ['red'], 2, ['red', 'giant-leap'])).toEqual(
            false
          );
        });
      ...
      describe('on the target row', () => {
        it('fades other dots', () => {
          expect(isDotFaded(1, ['giant-leap'], 1, ['red'])).toEqual(true);
        });

        it('shows the value of the hovered dot', () => {
          expect(isValueHidden(1, ['red'], 1, ['red'])).toEqual(false);
        });

        it('hides the values for the other dots', () => {
          expect(isValueHidden(1, ['red'], 1, ['giant-leap'])).toEqual(true);
        });
      });
    });
    ...
    describe('on a stacked data point', () => {
      describe('on other rows', () => {
        it('filters stacked dots to the hovered color', () => {
          expect(
            getDotColors(1, ['red', 'terra-form'], 2, [
              'red',
              'terra-form',
              'giant-leap',
            ])
        ...
        it('hides dots of different colors', () => {
          expect(
            isDotHidden(1, ['red', 'giant-leap'], 2, ['terra-form'])
          ).toEqual(true);
        });

        it('hides values of the same color for single dots', () => {
          expect(isValueHidden(1, ['red', 'terra-form'], 2, ['red'])).toEqual(
            true
          );
        });
        ...
        it('hides values of the same color for stacked dots', () => {
          expect(
            isValueHidden(1, ['red', 'terra-form'], 2, ['red', 'terra-form'])
          ).toEqual(true);
        });
      ...
      describe('on the target row', () => {
        it('fades other dots', () => {
          expect(
            isDotFaded(1, ['giant-leap', 'terra-form'], 1, ['red'])
          ).toEqual(true);
        });

        it('shows the value of the hovered dot', () => {
          expect(
            isValueHidden(1, ['red', 'terra-form'], 1, ['red', 'terra-form'])
          ).toEqual(false);
        });
