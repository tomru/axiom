...
const toValues = (words) => words.map((word) => ({
  text: word,
  value: (Math.random() * 80) + 10,
}));

const linearScale = ([min, max], [lower, upper]) => (value) =>
  min === lower || min === max ? lower :
    lower + ((upper - lower) / (max - min)) * (value - min);

const getPositions = (words) => new Promise((resolve, reject) => {
  try {
    const width = 1000;
    const height = 600;
...
export default class Documentation extends Component {
  ...
  initWordCloud(data = this._lastData) {
    this._lastData = data;
    getPositions(data).then(({ labels, boundsWidth, boundsHeight }) => {
      this.setState({
        boundsHeight,
        boundsWidth,
    ...
    return (
      <DocumentationContent>
        <DocumentationShowCase
          ...
          { error && (
            <AlertBar type="error">{ error }</AlertBar>
          ) }
                ...
                <Dropdown
                  ...
                  <DropdownTarget>
                    <Word
                        ...
                        onMouseLeave={ hoveredWord === text ? (() => this.setState({ hoveredWord: null })) : null }
                        text={ text }
                        x={ x }
                        y={ y } />
                  </DropdownTarget>
